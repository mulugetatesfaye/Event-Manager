...
export function EventCard({ event }: EventCardProps) {
  ...
  const getStatusBadge = () => {
    if (isPastEvent)
      return { label: t("status.ended"), variant: "secondary" as const };
    if (isFull)
      return { label: t("status.soldOut"), variant: "destructive" as const };
    if (isEventToday)
      return { label: t("status.today"), variant: "default" as const };
    if (isEventTomorrow)
      return { label: t("status.tomorrow"), variant: "default" as const };
    if (fillPercentage > 80)
      return { label: t("status.fillingFast"), variant: "default" as const };
    if (isUpcoming)
      return { label: t("status.available"), variant: "outline" as const };
    return null;
  };
          ...
          <div className="absolute top-3 left-3 right-3 flex items-center justify-between gap-2">
            ...
            {statusBadge && (
              <Badge
                ...
                className="text-xs font-medium bg-white text-gray-900"
              >
                {statusBadge.label}
              </Badge>
            )}
          ...
          {event.organizer && (
            <div className="pt-4 border-t border-gray-200">
              <div className="flex items-center gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-gray-500">{t("hostedBy")}</p>
